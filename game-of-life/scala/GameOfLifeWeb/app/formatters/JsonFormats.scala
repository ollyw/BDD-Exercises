...
object JsonFormats {
  ...
  implicit object LiveCellFormat extends Format[LiveCell] {
    def reads(json: JsValue): JsResult[LiveCell] = JsSuccess(LiveCell((json \ "row").as[Int],
    												(json \ "column").as[Int]))
  ...
  implicit object LiveCellSetFormat extends Format[LiveCellSet] {
	def reads(json: JsValue): JsResult[LiveCellSet] = JsSuccess(LiveCellSet(json.as[Seq[LiveCell]]: _*))
  ...
  implicit object GameFormat extends Format[Game] {
    def reads(json: JsValue): JsResult[Game] = {
    	...
    	JsSuccess(new Game(rows, columns))
    }
